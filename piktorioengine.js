// piktorioengine.js — Piktorioゲームエンジン v0.1
...
const Piktorioengine = (() => {
  ...
  function cellAction(r, c, zone) {
    ...
    if (zone === 'bottomright') {
      ...
      if (state.player.piks[color] <= 0) {
        _setMessage(`${_colorJa(color)}pikを持っていません`);
        _render();
        return;
      }
  ...
  function _render() {
    ...
    const CELL = Math.floor(Math.min(canvas.width / state.cols, canvas.height / state.rows));
    const ox   = Math.floor((canvas.width  - CELL * state.cols) / 2);
    const oy   = Math.floor((canvas.height - CELL * state.rows) / 2);

    ctx.clearRect(0, 0, canvas.width, canvas.height);

    for (let r = 0; r < state.rows; r++)
      for (let c = 0; c < state.cols; c++)
        _drawCell(ctx, r, c, CELL, ox, oy);
  ...
  function _drawCell(ctx, r, c, CELL, ox, oy) {
    ...
    const fs = Math.max(8, Math.floor(CELL * 0.27));
    ctx.font = `bold ${fs}px monospace`;

    if (cell.piks.red > 0) {
      ctx.fillStyle = '#ff6666';
      ctx.fillText(cell.piks.red, x + 2, y + fs + 1);
    }
    if (cell.piks.yellow > 0) {
      ctx.fillStyle = '#ffee44';
      const tw = ctx.measureText(cell.piks.yellow).width;
      ctx.fillText(cell.piks.yellow, x + CELL - tw - 2, y + fs + 1);
    }
    if (cell.piks.blue > 0) {
      ctx.fillStyle = '#55aaff';
      ctx.fillText(cell.piks.blue, x + 2, y + CELL - 3);
    }
  }
  ...
  function _drawActionRange(ctx, CELL, ox, oy) {
    ...
    dirs.forEach(([dr, dc]) => {
      ...
      if (state.map[r][c].type === TILE.WALL) return;
  ...
  function _drawPlayer(ctx, CELL, ox, oy) {
    ...
    if (piks.red > 0) {
      ctx.fillStyle = '#ff6666';
      ctx.fillText(piks.red, x + 2, y + fs + 1);
    }
    if (piks.yellow > 0) {
      ctx.fillStyle = '#ffee44';
      const tw = ctx.measureText(piks.yellow).width;
      ctx.fillText(piks.yellow, x + CELL - tw - 2, y + fs + 1);
    }
    if (piks.blue > 0) {
      ctx.fillStyle = '#55aaff';
      ctx.fillText(piks.blue, x + 2, y + CELL - 3);
    }
  }
  ...
  function handleCanvasClick(e) {
    ...
    const { r, c, lx, ly, CELL } = _pixelToCell(clientX, clientY);
    if (r < 0 || r >= state.rows || c < 0 || c >= state.cols) return;
