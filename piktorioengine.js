// piktorioengine.js — Piktorioゲームエンジン v0.5
...
const Piktorioengine = (() => {
  ...
  const TILE_FILL = { walk:'#2d5a1b', wall:'#5a3010', cracked:'#4a2808', red:'#6b1a1a', blue:'#1a4a6b', yellow:'#5a5a10' };
  const MINI_FILL = { walk:'#3a7a22', wall:'#7a4a20', cracked:'#6a3a18', red:'#993333', blue:'#2255aa', yellow:'#888820' };
  const ACTION_BG = { walk:'rgba(255,255,255,0.18)', cracked:'rgba(200,160,100,0.22)', red:'rgba(255,100,100,0.22)', blue:'rgba(80,160,255,0.22)', yellow:'rgba(240,220,60,0.22)' };

  let state = null;

  function init(mapData) {
    const rows=mapData.tiles.length, cols=mapData.tiles[0].length;
    // 霧半径：マップ全体が入る円（中心からの最大距離+1）
  ...
  function _enterActionPhase(){
    state.phase='player_action';_setMessage('アクション：マスをタップしてpikを操作');_render();
  }
  ...
  function _runPikTurn(){
    for(let r=0;r<state.rows;r++)for(let c=0;c<state.cols;c++){
      ...
      const dmg=cell.piks.red*2+cell.piks.blue*1+cell.piks.yellow*1;
      if(dmg<=0)continue;
  ...
  function _drawCell(ctx,r,c,x,y,CELL){
    ...
    if(cell.type==='cracked'){_drawCrackPattern(ctx,x,y,CELL,cell.hp);_drawHpGauge(ctx,x,y,CELL,cell.hp);}
    const fs=Math.max(9,Math.floor(CELL*0.26));ctx.font=`bold ${fs}px monospace`;
    if(cell.piks.red>0){ctx.fillStyle='#ff6666';ctx.fillText(cell.piks.red,x+3,y+fs+2);}
    ...
    if(cell.piks.blue>0){ctx.fillStyle='#55aaff';ctx.fillText(cell.piks.blue,x+3,y+CELL-4);}
  }

  function _drawCrackPattern(ctx,x,y,CELL,hp){
    const intensity=1-(hp??CRACKED_HP)/CRACKED_HP;
    const alpha=0.3+intensity*0.5;
    ctx.save();ctx.strokeStyle=`rgba(200,160,80,${alpha})`;ctx.lineWidth=1;
    const cx=x+CELL/2,cy=y+CELL/2;
    const cracks=[[[cx,cy],[cx-CELL*0.3,cy-CELL*0.35]],[[cx,cy],[cx+CELL*0.25,cy-CELL*0.3]],[[cx,cy],[cx+CELL*0.1,cy+CELL*0.4]],[[cx,cy],[cx-CELL*0.2,cy+CELL*0.3]]];
    const count=1+Math.floor(intensity*(cracks.length-1));
    for(let i=0;i<count;i++){ctx.beginPath();ctx.moveTo(cracks[i][0][0],cracks[i][0][1]);ctx.lineTo(cracks[i][1][0],cracks[i][1][1]);ctx.stroke();}
    ctx.restore();
  }
  ...
  function _drawHpGauge(ctx,x,y,CELL,hp){
    ...
    if(ratio>0){
      ...
      ctx.fillStyle=`hsl(${Math.floor(30*ratio)+20},70%,55%)`;ctx.fill();
    }
    ctx.beginPath();ctx.arc(cx,cy,r,0,Math.PI*2);ctx.strokeStyle='rgba(220,180,100,0.7)';ctx.lineWidth=1;ctx.stroke();
    const fs=Math.max(7,Math.floor(CELL*0.2));ctx.font=`bold ${fs}px monospace`;
    ctx.fillStyle='rgba(255,230,180,0.95)';ctx.textAlign='center';ctx.textBaseline='middle';
    ctx.fillText(hp,cx,cy+0.5);ctx.textAlign='left';ctx.textBaseline='alphabetic';
  }
  ...
  function _drawActionButtons(ctx,CELL,ox,oy){
    ...
    for(let dr=-1;dr<=1;dr++)for(let dc=-1;dc<=1;dc++){
      ...
      const x=ox+c*CELL,y=oy+r*CELL;
      if(dr===0&&dc===0)_drawPickupButton(ctx,x,y,CELL,type,cell);
      else _drawPlaceButton(ctx,x,y,CELL,type);
    }
  }

  function _drawPlaceButton(ctx,x,y,CELL,type){
    const cx=x+CELL/2,cy=y+CELL/2,r=CELL*0.28;
    ctx.beginPath();ctx.arc(cx,cy,r,0,Math.PI*2);
    ctx.fillStyle=ACTION_BG[type]||'rgba(255,255,255,0.15)';ctx.fill();
    ctx.strokeStyle='rgba(255,255,255,0.35)';ctx.lineWidth=1;ctx.stroke();
    ctx.beginPath();ctx.arc(cx,cy,r*0.35,0,Math.PI*2);ctx.fillStyle='rgba(255,255,255,0.7)';ctx.fill();
  }

  function _drawPickupButton(ctx,x,y,CELL,type,cell){
    if(!COLORS.some(col=>cell.piks[col]>0))return;
    const cx=x+CELL/2,cy=y+CELL/2,r=CELL*0.28;
    ctx.beginPath();ctx.arc(cx,cy,r,0,Math.PI*2);
    ctx.fillStyle=ACTION_BG[type]||'rgba(255,255,255,0.15)';ctx.fill();
    ctx.strokeStyle='rgba(255,255,255,0.35)';ctx.lineWidth=1;ctx.stroke();
    const s=r*0.55;
    ctx.save();ctx.strokeStyle='rgba(255,255,255,0.85)';ctx.lineWidth=CELL*0.07;ctx.lineCap='round';
  ...
  function _drawPlayer(ctx,CELL,ox,oy){
    ...
    if(piks.red>0){ctx.fillStyle='#ff6666';ctx.fillText(piks.red,x+3,y+fs+2);}
    if(piks.yellow>0){ctx.fillStyle='#ffee44';ctx.fillText(piks.yellow,x+CELL-ctx.measureText(piks.yellow).width-3,y+fs+2);}
    if(piks.blue>0){ctx.fillStyle='#55aaff';ctx.fillText(piks.blue,x+3,y+CELL-4);}
  }
  ...
  function _drawArrow(ctx,cellX,cellY,CELL,dcol,drow){
    ...
    ctx.fillStyle='rgba(255,255,220,0.88)';ctx.fill();ctx.strokeStyle='rgba(0,0,0,0.4)';ctx.lineWidth=1;ctx.stroke();
    ctx.restore();
  }

  function _drawCheckmark(ctx,cellX,cellY,CELL){
    const cx=cellX+CELL/2,cy=cellY+CELL/2,s=CELL*0.22;
    ctx.beginPath();ctx.arc(cx,cy,s*1.1,0,Math.PI*2);ctx.fillStyle='rgba(40,160,40,0.72)';ctx.fill();
    ctx.save();ctx.strokeStyle='rgba(255,255,255,0.95)';ctx.lineWidth=s*0.4;ctx.lineCap='round';ctx.lineJoin='round';
  ...
  function _drawMinimap(ctx,canvas){
    ...
    for(let r=0;r<state.rows;r++)for(let c=0;c<state.cols;c++){
      ...
      if(cell.piks.red>0||cell.piks.blue>0||cell.piks.yellow>0){
        let dc='#fff';
        if(cell.piks.red>0)dc='#ff6666';else if(cell.piks.yellow>0)dc='#ffee44';else dc='#55aaff';
  ...
  function _updateUI(){
    ...
    set('piktorioPhase',names[state.phase]||state.phase);set('piktorioTurn',`${state.turn}`);
    set('piktorioMoves',state.phase==='player_move'?`${state.movesLeft}`:'—');set('piktorioMessage',state.message);
  }
  ...
  function handleCanvasClick(e){
    ...
    if(state.phase==='player_action'){
      if(r<0||r>=state.rows||c<0||c>=state.cols)return;
      const half=CELL/2;let zone;
      if(lx<half&&ly<half)zone='topleft';else if(lx>=half&&ly<half)zone='topright';
      else if(lx<half&&ly>=half)zone='bottomleft';else zone='bottomright';
      cellAction(r,c,zone);
    }
  }
