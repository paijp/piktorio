// piktorioengine.js — Piktorioゲームエンジン v0.3
...
const Piktorioengine = (() => {
  ...
  const MOVE_LIMIT = 3;
  const MAP_SCALE  = 2;
  const FOG_RADIUS = MOVE_LIMIT + 1; // ユークリッド距離
    ...
    for (let dr = -ri; dr <= ri; dr++) {
      for (let dc = -ri; dc <= ri; dc++) {
        ...
        if (nr>=0 && nr<state.rows && nc>=0 && nc<state.cols) {
          if (d2 < state.fogMap[nr][nc]) state.fogMap[nr][nc] = d2;
        }
      }
    }
  }
  ...
  function _collectPiks() {
    ...
    COLORS.forEach(color => { state.player.piks[color] += cell.piks[color]; cell.piks[color] = 0; });
  }
  ...
  function _enterActionPhase() {
    ...
    _setMessage('アクション：マスをタップしてpikを操作');
    _render();
  }
  ...
  function cellAction(r, c, zone) {
    ...
    if (zone === 'bottomright') {
      if (type !== 'walk') { _setMessage('色専用マスからは直接拾えません'); _render(); return; }
  ...
  function _render() {
    ...
    if (state.phase==='player_action') _drawActionRange(ctx, CELL, ox, oy);
    ...
    if (state.phase==='player_move') {
      // 上下左右に△
      [[0,-1],[0,1],[-1,0],[1,0]].forEach(([dcol,drow]) => _drawArrow(ctx, cellX, cellY, CELL, dcol, drow));
      // プレイヤー上に✓
  ...
  function _drawArrow(ctx, cellX, cellY, CELL, dcol, drow) {
    ...
    const cx = cellX + CELL/2 + dcol*CELL*0.8;
    const cy = cellY + CELL/2 + drow*CELL*0.8;
    const s  = CELL * 0.18;
    ctx.save();
    ctx.translate(cx, cy);
    ctx.rotate(Math.atan2(drow, dcol) + Math.PI/2);
    ctx.beginPath();
    ctx.moveTo(0, -s);
    ctx.lineTo( s, s*0.6);
    ctx.lineTo(-s, s*0.6);
  ...
  function _drawCheckmark(ctx, cellX, cellY, CELL) {
    ...
    const cy = cellY - s * 0.8;
    ctx.beginPath();
    ctx.arc(cx, cy, s*0.9, 0, Math.PI*2);
    ctx.fillStyle = 'rgba(70,180,70,0.9)';
    ctx.fill();
  ...
  function _drawActionRange(ctx, CELL, ox, oy) {
    ...
    for (let dr=-1;dr<=1;dr++) for (let dc=-1;dc<=1;dc++) {
      ...
      if (state.map[r][c].type==='wall') continue;
  ...
  function _bfsReachable(startR, startC, steps) {
    const vis=new Set([`${startR},${startC}`]), q=[[startR,startC,steps]], res=[];
    while (q.length) {
      const [r,c,left]=q.shift();
      if (!left) continue;
  ...
  function _updateUI() {
    ...
    const eb=document.getElementById('piktorioEndTurn'), sb=document.getElementById('piktorioSkipMove');
    if (eb) eb.disabled = state.phase!=='player_action';
    if (sb) sb.disabled = state.phase!=='player_move';
  }
  ...
  function _hitCheckmark(px, py, cellX, cellY, CELL) {
    const s=CELL*0.2, cx=cellX+CELL/2, cy=cellY-s*0.8;
    const dx=px-cx, dy=py-cy;
    return dx*dx+dy*dy <= s*s*2.25; // r*1.5の円
  }
  ...
  function handleCanvasClick(e) {
    ...
    const pr=state.player.row, pc2=state.player.col;
    const cellX=ox+pc2*CELL, cellY=oy+pr*CELL;

    // ✓ チェックマークヒット
    if ((state.phase==='player_move'||state.phase==='player_action') &&
         _hitCheckmark(px,py,cellX,cellY,CELL)) {
