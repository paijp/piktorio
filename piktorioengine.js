// piktorioengine.js — Piktorioゲームエンジン v0.4
// GitHub Pages対応・サーバレス・ターン制シミュレーション

const Piktorioengine = (() => {

  const TILE = { WALK:'walk', WALL:'wall', RED:'red', BLUE:'blue', YELLOW:'yellow' };
  const PHASE = { PLAYER_MOVE:'player_move', PLAYER_ACTION:'player_action', PIK:'pik', ENEMY:'enemy' };
  const COLORS = ['red','blue','yellow'];
  const MOVE_LIMIT  = 3;
  const MAP_SCALE   = 2;
  const FOG_RADIUS  = MOVE_LIMIT + 1;

  const TILE_FILL = { walk:'#2d5a1b', wall:'#5a3010', red:'#6b1a1a', blue:'#1a4a6b', yellow:'#5a5a10' };
  const MINI_FILL = { walk:'#3a7a22', wall:'#7a4a20', red:'#993333', blue:'#2255aa', yellow:'#888820' };
  ...
  function _revealFog(r,c) {
    const R2=FOG_RADIUS*FOG_RADIUS, ri=Math.ceil(FOG_RADIUS);
  ...
  function _enterActionPhase() {
    ...
    _setMessage('アクション：マスをタップしてpikを操作');_render();
  }
  ...
  function _runPikTurn(){state.phase='enemy';_setMessage('敵ターン中…');_render();setTimeout(_runEnemyTurn,400);}
  ...
  function _drawMinimap(ctx,canvas) {
    const MINI=5,PAD=8;
    const mW=state.cols*MINI,mH=state.rows*MINI;
    ...
    for(let r=0;r<state.rows;r++) for(let c=0;c<state.cols;c++){
      ...
      if(cell.piks.red>0||cell.piks.blue>0||cell.piks.yellow>0){
        let dc='#fff';
        if(cell.piks.red>0) dc='#ff6666';
        else if(cell.piks.yellow>0) dc='#ffee44';
        else dc='#55aaff';
        ctx.beginPath();ctx.arc(x+MINI*0.5,y+MINI*0.5,MINI*0.3,0,Math.PI*2);
        ctx.fillStyle=dc;ctx.fill();
      }
    }

    const px=mx+state.player.col*MINI+MINI*0.5,py=my+state.player.row*MINI+MINI*0.5;
    ctx.beginPath();ctx.arc(px,py,MINI*0.55,0,Math.PI*2);
    ctx.fillStyle='#f0efe0';ctx.fill();
  ...
  function _drawActionRange(ctx,CELL,ox,oy){
    ...
    for(let dr=-1;dr<=1;dr++) for(let dc=-1;dc<=1;dc++){
      ...
      ctx.fillStyle='rgba(180,255,180,0.14)';ctx.fillRect(ox+c*CELL,oy+r*CELL,CELL,CELL);
    }
  }
  ...
  function _drawCheckmark(ctx,cellX,cellY,CELL){
    ...
    ctx.beginPath();ctx.moveTo(cx-s*0.45,cy+s*0.0);ctx.lineTo(cx-s*0.1,cy+s*0.45);ctx.lineTo(cx+s*0.48,cy-s*0.42);
